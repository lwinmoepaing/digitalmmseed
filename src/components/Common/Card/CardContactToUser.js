...
const CardContactToUser = ({ payload, edit }) => {
  const profileImage = `${BASE_API_URL}${payload.user.image}`
  const headImage = `${BASE_API_URL}${payload.headImg}`
  // const router = useRouter()
  const goDetail = () => {
    if (edit === true) return
    Router.push(`/farmer/user-projects/_id?id=${payload._id}`, `/farmer/user-projects/${payload._id}`, { shallow: true })
  }
...
export default CardContactToUser
